...
export function glue_with_fixed_point(s, ep1, ep2, data) {
    ...
    if (p1.equals(p2, EPS.TINY)) {
        for (let i = 0; i < rcp1.length; i++) {
            if (rcp2.includes(rcp1[i]))
                throw new Error("Can't glue linked parts");
        }
    }
    ...
    const cb =
        typeof data.points == "string" ? default_data_callback : data.points;
    ...
    const glue_lines =
        s.lines_by_key("__glue_line")[true] || merged_pt.common_lines(fixed);
    ...
    } else if (!data.points.startsWith("delete")) {
        // Merge Case
        assert(
...
export function glue(s, ep1, ep2, data) {
    ...
    ep2[0]
        .connected_component()
        .transform((pt) => pt.move_to(transform_fun(pt)));
    ...
    const glue_lines =
        s.lines_by_key("__glue_line")[true] ||
        merged_pts[0].common_lines(merged_pts[1]);
    s.remove_underscore_attributes("__glue_line");
    ...
    if (
        data.lines == "delete" ||
        (typeof data.points == "string" && data.points.startsWith("delete"))
    ) {
        s.remove_lines(...glue_lines);
    } else if (
        !(typeof data.points == "string" && data.points.startsWith("delete"))
...
function delete_glue_point(s, pt, line_callback) {
    ...
    if (adjacent.length > 2)
        throw new Error(
            "Cant safely delete glue point! To many adjacent lines",
        );
...
function restricted_component_pts(p1, fixed) {
    ...
    while (to_visit_points.length > 0) {
        ...
        for (const l of lines) {
            ...
            if (
                visited_points.indexOf(other_endpoint) < 0 &&
                to_visit_points.indexOf(other_endpoint) < 0
            ) {
