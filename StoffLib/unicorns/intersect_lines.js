...
function intersect_lines(sketch, line1, line2){
    ...
    for (let i = 0; i < intersections.length; i++){
        intersections[i][4] = sketch.add_point(intersections[i][4]).set_color(int_color);
    }
    ...
    for (let i = 0; i < intersections.length - 1; i++) {
        ...
        const new_first_pt = rel_subslice[0].mult(1 - ifrac).add(rel_subslice[1].mult(ifrac));
        const new_last_pt = rel_subslice[sl_len - 2].mult(1 - ifracNext)
...
function _clean_intersection_positions(intersection_positions, line1){
    ...
    for (let i = 0; i < intersection_positions.length; i++){
        ...
        const rel_position = p0.mult(1 - current_entry[4]).add(p1.mult(current_entry[4]));
        cleaned_ip.push([current_entry[0], current_entry[2], current_entry[4], current_entry[5], l1_to_abs(rel_position)]);
    }
    return cleaned_ip;
}
...
function _line_segments_intersect(start1, end1, start2, end2) {
    ...
    if (Math.abs(denominator) < EPS.FINE) {
        ...
        const abs_intersection_pos = start1.mult(1 - relX_slice).add(end1.mult(relX_slice));
