...
`StoffLib/geometry.js` is a self contained file with my useful functionalities concerning Vectors and 2D-computations.
...
- `.mult(el : number | Vector | Matrix) : Vector | number | Vector`: Multiplies the vector by a scalar, vector, or matrix.
```javascript
    const v = new Vector(2, 3);
...
- `.rotate(angle : number) : Vector`: Rotates the vector by a given angle. The angle is given in radiants.
```javascript
...
Note that no function except `set` mutates the original vector.
...
Creates an orthogonal transformation function that rotates and stretches `v1` to `v2`.
...
const v1 = new Vector(1, 0);
const v2 = new Vector(0, 1);
console.log(vec_angle_clockwise(v1, v2)); // Clockwise angle in radians
```
...
```javascript
console.log(deg_to_rad(180)); // Radians equivalent of 180 degrees
```
...
```javascript
console.log(rad_to_deg(Math.PI)); // Degrees equivalent of PI radians
```
