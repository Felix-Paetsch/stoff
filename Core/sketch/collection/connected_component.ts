...
export class ConnectedComponent {
    ...
    group_by_key(key: string) {
        const { points, lines } = this.obj();
        const groupedPoints: Record<string, Point[]> = points.reduce(
            (acc: Record<string, Point[]>, pt: Point) => {
                const groupKey =
                    pt.data[key] !== undefined ? pt.data[key] : "_";
                if (!acc[groupKey]) {
                    acc[groupKey] = [];
                }
        ...
        const groupedLines: Record<string, Line[]> = lines.reduce(
...
export class AvoidantConnectedComponent extends ConnectedComponent {
    constructor(
        ...
        expect(same_sketch(root_el, ...avoids));
        expect(!avoids.includes(root_el));
    }
