...
export function construct_base_tshirt_parts(
    ...
    // Out.put(new Recording.LiveRecording(sketch), "#live");
...
function Uebergang(sketch: Sketch, mea: BoundShirtSideMeasurements) {
    ...
    const shoulder_line = CollectionMethods.get_line_between_points(sketch, {
        point1: { name: "c" },
        point2: { name: "d" },
    });
    ...
    if (shoulder_line != null) {
        ...
        const H_main = CollectionMethods.get_point(sketch, {
            name: "h",
            position: "main",
        })!;
        const H_side = CollectionMethods.get_point(sketch, {
            name: "h",
            position: "side",
        })!;
        ...
        const waistlines = CollectionMethods.get_lines(sketch, {
            name: "waistline",
        });

        waistlines.map((l) => {
            const cut_res = cut(l);
            const cc = CollectionMethods.connected_component(
                sketch,
                cut_res[0].points[0]!,
            );
            CollectionMethods.map(cc, (v) => v.add(new Vector(0, 5)));
        });
...
function add_curve(sketch: Sketch) {
    ...
    const normalA = lA
        ...
        .orthogonal()
        .invert();
    const velocitiesDA = [tangentD, normalA].map((v) =>
