...
import { Vector, polygon_orientation, VERTICAL } from "../StoffLib/geometry";

import {
    cut_with_fixed_point,
    cut_without_fixed_point,
    cut_along_line_path,
} from "./sketch_methods/cut";
import { glue_with_fixed_point, glue } from "./sketch_methods/glue";

import Point from "../StoffLib/point";
import ConnectedComponent from "../StoffLib/connected_component";
import { default_data_callback } from "../StoffLib/copy";
...
export default class SewingSketch extends Sketch {
    ...
    cut(line, fixed_pt = null, grp1 = "smart", grp2 = "smart") {
        ...
        */
        ...
        if (line instanceof Array) {
            ...
            line =
                line[0].common_line(line[1]) ||
                this.line_between_points(line[0], line[1]);
        }
        ...
        if (fixed_pt instanceof Point)
            return cut_with_fixed_point(this, line, fixed_pt, grp1, grp2);
    ...
    glue(ident1, ident2, data = {}) {
        ...
        */
        ...
        assert(!ident1[0].equals(ident1[1]));
        assert(!ident2[0].equals(ident2[1]));
        ...
        if (ident1[0] == ident2[0])
            return glue_with_fixed_point(this, ident1, ident2, data);
    ...
    anchor(...objects) {
        ...
        if (objects.length > 0) {
            connected_components = connected_components.filter((c) => {
                return (
                    c.points.some((p) => pts.includes(p)) ||
                    c.lines.some((l) => lns.includes(l))
                );
            });
        }

        if (connected_components.length == 0)
            throw new Error("Nothing to anchor!");
    ...
    oriented_component(el) {
        ...
        */
        if (el instanceof ConnectedComponent)
            return this.oriented_component(el.root_el);
        ...
        if (el instanceof Array) {
            ...
            while (el.length > 0) {
                ...
                if (!smth_changed)
                    throw new Error("Component lines don't form cycle.");
                smth_changed = true;
            }
        ...
        for (let i = 1; i < lines.length; i++) {
            lines_with_orientation.push({
                ...
                orientation: lines_with_orientation[i - 1].orientation
                    ? lines[i - 1].p2 == lines[i].p1
                    : lines[i - 1].p1 == lines[i].p1,
