...
export function findFaces(lines: Line[]) {
    ...
    const CC_faces = connected_components.map((cc: SketchElement[]) =>
        findConnectedComponentFaces(CollectionMethods.get_lines(cc)),
    );
    return new FaceAtlas(CC_faces);
}

function findConnectedComponentFaces(
    lines: Line[],
): ConnectedComponentFaceData {
    const boundaries: {
        lines: Line[];
        orientation: boolean[];
    }[] = [];
    ...
    for (const line of lines) {
        ...
        outerLoop: for (const searching_with_orientation of [true, false]) {
            ...
            let latest_endpoint = line.endpoint_from_orientation(
                !searching_with_orientation,
            ); // The one after current line, before next line
            ...
            while (true) {
                ...
                for (let i = 0; i < visited_lines.length; i++) {
                    if (
                        visited_lines[i]!.endpoint_from_orientation(
                            orientations[i]!,
                        ) == latest_endpoint
                    ) {
                        visited_lines.push(latest_line);
                        const latest_orientation =
                            latest_line.p2 == latest_endpoint;
                ...
                const possible_next_lines = lines
                    .filter(
                        (l) =>
                            l.has_endpoint(latest_endpoint) &&
                            lines_map.has(l) &&
                            !visited_lines.includes(l) &&
                            latest_line !== l &&
                            !lines_map.get(l)![
                                l.p1 == latest_endpoint
                                    ? "with_orientation"
                                    : "against_orientation"
                            ],
                    )
                    .sort((l1, l2) => {
                        const order = compare_lines_at_endpoint(
                            l1,
                            l2,
                            latest_line,
                            latest_endpoint,
                ...
                if (possible_next_lines.length == 0) {
                    ...
                    orientations.pop();
                    latest_endpoint =
                        latest_line.other_endpoint(latest_endpoint);
                    latest_line = visited_lines.pop()!;
                    continue;
                } else {
                    visited_lines.push(latest_line);
...
function compare_lines_at_endpoint(
    l1: Line,
    l2: Line,
    latest_line: Line,
    latest_endpoint: Point,
) {
    ...
    let vec1 = l1.get_tangent_vector(l1.position_at_fraction(0.05)).scale(-1);
    if (latest_endpoint == l1.p2) {
        vec1 = vec1.scale(-1);
    }
    let vec2 = l2.get_tangent_vector(l2.position_at_fraction(0.05)).scale(-1);
    if (latest_endpoint == l2.p2) {
        vec2 = vec2.scale(-1);
    }

    const diff =
        vec_angle_clockwise(rel_to, vec1, ZERO, true) -
        vec_angle_clockwise(rel_to, vec2, ZERO, true);
    return diff;
}
