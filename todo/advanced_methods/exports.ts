...
import {
    GlueIdent,
    GlueResult,
    glue as glue_with_validated_input,
} from "./glue";
...
export function anchor(s: Sketch, ...objects: SketchElement[]) {
    ...
    let connected_component_points = s
...
export function path_between_points(
    ...
    while (last_line_p2 !== p2) {
        ...
        expect(new_line instanceof Line, "There is no path from p1 to p2.");

        lines.push(new_line);

        expect(last_line_p2 != p1, "There is no path from p1 to p2.");
...
export function unfold(
    ...
    expect(same_sketch(...along_line, s));
