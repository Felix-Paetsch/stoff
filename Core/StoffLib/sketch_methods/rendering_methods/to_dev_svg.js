...
function create_dev_svg_from_sketch(s, width = null, height = null) {
    ...
    const createCircle = (point) => {
        ...
        svgContent += `<circle cx="${point.x}" cy="${point.y}" r="${radius}" stroke="${stroke}" fill="${fill}" opacity="${opacity}" stroke-width="${strokeWidth}" x-data="${JSON.stringify(
            point_data
        )
            .replace(/\\/g, "\\\\")
            .replace(/"/g, "&quot;")}" hover_area/>`;
    ...
    const createPolyline = (polyline) => {
        ...
        const pointsString = polyline.sample_points
        ...
        if (typeof line_data === "object") {
            line_data._length =
                Math.round(polyline.original_line.get_length() * 1000) / 1000;
        }

        // Hover area
        svgContent += `<polyline points="${pointsString}" style="fill:none; stroke: rgba(0,0,0,0); stroke-width: ${Math.max(strokeWidth, 8)}" x-data="${JSON.stringify(
            line_data
        )
            .replace(/\\/g, "\\\\")
            .replace(/"/g, "&quot;")}" hover_area/>`;
...
export function data_to_serializable(data) {
    ...
    function nesting_buffer(data) {
        ...
        // Basic Stuff
        if (
            ["boolean", "number", "bigint", "string", "symbol"].includes(
                typeof data
            )
        ) {
            nesting--;
