...
function construct_main_body(sketch: Sketch, mea: BoundShirtSideMeasurements) {
    ...
    const P1 = sketch.add_point(B.subtract(new Vector(0, mea("shoulder"))));
    ...
    let len2 = Math.sqrt(
        Math.pow(mea("shoulder_length"), 2) - Math.pow(C.y - P1.y, 2),
    );

    const D = sketch.add_point(C.add(new Vector(len2, P1.y - C.y)));
    D.data.name = "d";
