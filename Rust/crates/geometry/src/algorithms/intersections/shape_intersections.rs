use crate::{
    ...
    intersections::{
        ShapeIntersection,
        utils::{deduped_intersections, is_shape_end, shapes_are_parallel_at_position},
    },
    length_map::LengthMap,
    utils::length_recursion::{
        index::{half_shape, quater_shapes, shape_cant_get_within_x},
...
pub fn find_shape_intersections(
    ...
    find_shape_intersections_recursion(
        shape1,
        LengthRecursionData::new(shape1, lm1.lengths(), |v| {
            closest_point_on_shape_with_length_map(v, shape1, lm1.lengths())
                .unwrap()
                .distance
        }),
        shape2,
        LengthRecursionData::new(shape2, lm2.lengths(), |v| {
