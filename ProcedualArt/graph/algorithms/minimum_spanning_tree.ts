...
import {
    wasm_graph_minimum_spanning_tree_edge_list,
    wasm_graph_minimum_spanning_tree_of_vertices_edge_list,
    WASMCompatability,
} from "Rust/exports";
...
export function minimum_spanning_tree<E extends IntoLengthGraph>(on: E): E {
    ...
    const edges = WASMCompatability.Allocations.free_after_use(ser, (ser) =>
        wasm_graph_minimum_spanning_tree_edge_list(ser),
    );
...
export function minimum_spanning_tree_on_vertices(on: Vector[]): VertexGraph {
    ...
    const edges = wasm_graph_minimum_spanning_tree_of_vertices_edge_list(arr);
