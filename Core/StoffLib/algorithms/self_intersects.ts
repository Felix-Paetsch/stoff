...
export function self_intersects(line: Line): boolean {
    ...
    const points = line.sample_points;
    ...
    for (let i = 0; i < segCount - 1; i++) {
        ...
        const x1max =
            l1_last[0].x > l1_last[1].x ? l1_last[0].x : l1_last[1].x;
        ...
        for (let j = i + 1; j < segCount; j++) {
            ...
            while (
                ...
                const [intersects, intersectionPoint] =
                    line_segments_intersect(L1, L2);
                if (intersects) {
