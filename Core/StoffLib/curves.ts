...
function bezier_spline(
    ...
    const res_fun: Partial<SplineFunction> = function (t: Fraction) {
        ...
        return bezier(segments[segment_index]!, segment_t).to_array();
    };

    res_fun.points = static_points;
    res_fun.plot_control_points = bezier_plot_control_points.bind(
        res_fun as SplineFunction,
    );
...
function bezier_plot_control_points(
    ...
    for (let i = 0; i < pts.length - 1; i++) {
        lns.push(
            sketch
                .line_between_points(pts[i]!, pts[i + 1]!)
                .set_color("rgba(0,100,0,.3)"),
        );
    }
...
function hermite_plot_control_points(
    ...
    for (let i = 0; i < pts.length / 2; i++) {
        const ln = sketch
            .line_between_points(pts[2 * i]!, pts[2 * i + 1]!)
            .set_color("rgba(0,100,0,.3)");
        lns.push(ln);
    }
