...
export class Face {
    ...
    get_points(): Point[] {
        return Array.from(
            new Set(this.boundary.flatMap((l) => l.get_endpoints())),
        );
    }

    component(): ConnectedFaceComponent {
        return this.faceAtlas
            ? this.faceAtlas.connectedComponents.find(
                  (c) => c.faces.includes(this) || c.component == this,
              )!
            : this.own_component();
    }
    ...
    boundary_handedness(): boolean[] {
        ...
        for (let i = 0; i < this.boundary.length; i++) {
            const total =
                Number(this.boundary[i]!.right_handed) +
                Number(clockwise) +
                Number(orientations[i]);
    ...
    line_handedness(l: Line): boolean {
        ...
        const hand =
            this.boundary_handedness()[this.boundary.indexOf(l)] || false;
    ...
    is_adjacent(other: Face | Line | Point | RogueComponent): boolean {
        ...
        return (
            this !== other && other.boundary.some((l) => this.is_adjacent(l))
        );
    }

    contains(
        thing: Point | Vector | Line | Face | RogueComponent,
        only_interior: boolean = false,
    ): boolean {
        ...
        if (thing instanceof RogueComponent) {
            return this.contains(
                thing.lines[0]!.position_at_fraction(0.5)!,
                true,
            );
        }
        if (
            thing instanceof Point &&
            !only_interior &&
