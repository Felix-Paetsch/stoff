...
import * as CollectionMethods from "../collection/index.js";
...
export class FaceAtlas {
    ...
    constructor(connectedComponents: ConnectedComponentFaceData[]) {
        ...
        this.maximalComponents = this.connectedComponents.filter(
            (component) => !component.parent_component,
        );
        this.lines = Array.from(
            new Set(
                this.faces
                    .flatMap((f) => f.get_lines())
    ...
    get_points() {
        return CollectionMethods.unique(
            this.lines.flatMap((l) => l.get_endpoints()),
        );
    }
    ...
    adjacent_faces(
        line: Line,
    ): [Face, Face] | [RogueComponent, Face | null] | null {
        if (!this.lines.includes(line)) return null;

        const chain = this.rogueChains.find((c) =>
            c.get_lines().includes(line),
        )!;
        if (chain)
            return [
                chain,
    ...
    component_from_face(face: Face) {
        const comp = this.connectedComponents.filter(
            (c) => c.faces.includes(face) || c.component == face,
        )[0]!;
    ...
    is_boundary_line(line: Line): boolean {
        return this.maximalComponents.some((c) =>
            c.component?.get_lines().includes(line),
        );
    }
