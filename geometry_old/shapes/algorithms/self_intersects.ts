...
import { line_segments_intersect } from "../../algorithms";
import { Vector } from "../../classes";

// TODO
export function self_intersects(points: Vector[]): boolean {
    // Needs more work

    // const cached = line.self_intersection_cache;
        ...
        while (i < plen) {
            ...
            while (i + 1 < plen) {
                ...
                if (
                    ...
                    (x_dec && nx_diff >= 0) ||
                    (x_diff === 0 &&
                        ((y_inc && ny_diff <= 0) || (y_dec && ny_diff >= 0)))
                ) {
                    break;
                }
    ...
    for (let si = 0; si < segCount; si++) {
        ...
        for (let j = pi + 1; j <= e; j++) {
            ...
            while (j <= e && points[pi]!.equals(points[j]!)) j++;
            if (j > e) break;
    ...
    for (let i = 0; i < segCount - 1; i++) {
        ...
        const l1_first = lines1[0],
            l1_last = lines1[lines1.length - 1];
        const x1min =
            l1_first[0].x < l1_first[1].x ? l1_first[0].x : l1_first[1].x;
        ...
        for (let j = i + 1; j < segCount; j++) {
            ...
            const l2_first = lines2[0],
                l2_last = lines2[lines2.length - 1];
            const x2min =
                l2_first[0].x < l2_first[1].x ? l2_first[0].x : l2_first[1].x;
            const x2max =
                l2_last[0].x > l2_last[1].x ? l2_last[0].x : l2_last[1].x;
            ...
            while (
                ...
                const L1_lx = L1[l1l].x,
                    L1_rx = L1[l1r].x;
                const L2_lx = L2[l2l].x,
                    L2_rx = L2[l2r].x;
                ...
                if (intersects) {
                    ...
                    if (!isSharedEndpoint) {
                        // line.self_intersection_cache.self_intersects = true;
                        return true;
                    }
                }

                // Advance based on rightmost point
                if (L1_rx < L2_rx) cl1 += inc1;
                else cl2 += inc2;
            }
        }
    }

    // line.self_intersection_cache.self_intersects = false;
    return false;
}
