...
export class Vector {
    ...
    constructor(x: number | Vector, y: number = 0, column: boolean = true) {
    ...
    static equals(vec1: Vector, vec2: Vector, eps: number = EPS.MINY) {
        return vec1.equals(vec2, eps);
    }
    ...
    to_len(a: number) {
        expect(this.length() > EPS.STRICT_EQUAL, "Vector is (almost) 0");
...
export class Ray {
    ...
    constructor(src: Vector | [Vector, Vector], direction?: Vector) {
        ...
        expect(src instanceof Vector && direction instanceof Vector);
        expect(direction!.length() > 0, "Direction is zero!");
    ...
    get_orthogonal(at: Vector = ZERO): Line {
        return Line.from_direction(
            at,
            this.src.subtract(this.direction).get_orthogonal(),
        );
    }
