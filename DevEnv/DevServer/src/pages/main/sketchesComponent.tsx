...
export function SketchesComponent({ design }: SketchesProps) {
    ...
    useEffect(() => {
        ...
        async function processStack() {
            if (!(design instanceof Error)) {
                if (!cancelled) setMappedStack(null);
                return;
            }

            try {
                const mapped = await mapStackTrace(design);
                if (cancelled) return;

                const deletedTrace = deleteStackTraceFromFirstTSX(mapped);

                const mappedTrace = deletedTrace
                    .split("\n")
                    .map((l) => l.split("@"))
                    .map((l) => `${l[0] ?? ""}@${l[2] ?? ""}`)
                    .join("\n");

                if (!cancelled) setMappedStack(mappedTrace);
            } catch {
    ...
    if (design instanceof Error) {
        ...
        const renderer = new Renderer(design.result);
        ...
        if (design.data) {
            try {
                data = JSON.stringify(design.data, null, 2);
            } catch {
                data = "Failed to serialize data!";
            }
        }

        processedResult = { renderer, data };
    }
