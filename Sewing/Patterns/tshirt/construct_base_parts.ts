...
function construct_main_body(sketch: Sketch, mea: BoundShirtSideMeasurements) {
    ...
    const P3 = sketch.add_point(
        B.vec.subtract(new Vector(0, mea("side_height"))),
    );
...
function construct_side(
    ...
    const P1 = sketch.add_point(R.vec.subtract(new Vector(-len, 0)));
...
function lengthen_type1(sketch: Sketch, mea: BoundShirtSideMeasurements) {
    ...
    const FSLine = CollectionMethods.get_lines_between_points(sketch, {
        point1: {
            name: "f",
        },
        point2: {
    ...
    const BFLine = CollectionMethods.get_lines_between_points(sketch, {
        point1: {
            name: "f",
        },
        point2: {
    ...
    const FMain = CollectionMethods.get_points(sketch, {
        name: "f",
        position: "main",
    })[0]!;
    const FSide = CollectionMethods.get_points(sketch, {
        name: "f",
        position: "side",
    })[0]!;
    const S = CollectionMethods.get_points(sketch, { name: "s" })[0]!;

    const N = sketch.add_point(B.vec.add(new Vector(0, len)));
    N.data = {
        name: "n",
        position: "main",
    };
    const M = sketch.add_point(FMain.vec.add(new Vector(-y, len)));
    M.data = {
        name: "m",
        position: "main",
    };
    const O = sketch.add_point(FSide.vec.add(new Vector(y, len)));
    O.data = {
        name: "o",
        position: "side",
    };
    const P = sketch.add_point(S.vec.add(new Vector(-x, len)));
    P.data = {
        name: "p",
        position: "side",
    };
...
function lengthen_type2(sketch: Sketch, mea: BoundShirtSideMeasurements) {
    const lines = CollectionMethods.get_lines_between_points(sketch, {
        point1: { name: "h" },
        point2: { name: "c" },
    });
...
function align_lines(sketch: Sketch, l1: Line, l2: Line) {
    ...
    pts.reverse().forEach((p) => {
        p.move_to(transform(p.vec));
    });
}

function add_curves(sketch: Sketch) {
    const C = CollectionMethods.get_point(sketch, {
        name: "c",
        position: "side",
    })!;
    const R = CollectionMethods.get_point(sketch, { name: "r" })!;

    const tangentC = C.adjacent_lines()[0]!.shape.tangent_line(C.vec)!;
    const normalR = R.adjacent_lines()[0]!.shape.normal_line(R.vec)!;

    const intersection = tangentC.intersect(normalR)!;

    const shape_fn = Spline.bezier([C.vec, intersection, R.vec]);
    // Visualize.bezier_controlpoints(sketch, [C.vec, intersection, R.vec]);

    // const shape_fn = Spline.hermite([C.vec, R.vec], [tangentC, normalR]);
    // Visualize.hermite_controlpoints(
    //     sketch,
    //     [C.vec, R.vec],
    //     [tangentC, normalR],
    // );
    ...
    const A = CollectionMethods.get_point(sketch, {
        name: "a",
    })!;
    const D = CollectionMethods.get_point(sketch, { name: "d" })!;

    const normalA = A.adjacent_lines()[0]!.shape.normal_line(C.vec)!;
    const normalD = D.adjacent_lines()[0]!.shape.normal_line(D.vec)!;

    const intersection_AD = normalA.intersect(normalD)!;

    const shape_fn_AD = Spline.bezier([A.vec, intersection_AD, D.vec]);
    sketch.add_line(Polyline.from_function(shape_fn_AD), A, D);
}
