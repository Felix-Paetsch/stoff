...
function affine_transform_from_normalized_to_anchors(anchor1, anchor2) {
    return affine_transform_from_input_output(
        [CONST_RELATIVE_ANCHOR_1, CONST_RELATIVE_ANCHOR_2],
        [anchor1, anchor2]
    );
}
...
function rotation_fun(rotation_point, angle) {
    ...
    return (v) => {
        return rotMatrix.mult(v.subtract(rotation_point)).add(rotation_point);
    };
}
...
function vec_angle_clockwise(vec1, vec2) {
    ...
    return (
        scalar *
        Math.acos(
            v_reference.dot(v_target) /
                (v_reference.length() * v_target.length())
        )
    );
}

function const_relative_anchor_angle() {
    return vec_angle_clockwise(
        CONST_RELATIVE_ANCHOR_1,
        CONST_RELATIVE_ANCHOR_2
    );
}
