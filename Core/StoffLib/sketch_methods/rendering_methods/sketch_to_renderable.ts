...
import { get_bounding_box } from "../../collection.js";
...
function calculate_correct_width_height(
    ...
    */
    ...
    if (width == null && height !== null) {
        width =
            aspect_ratio * (height - 2 * CONF.DEFAULT_SAVE_PX_PADDING) +
            2 * CONF.DEFAULT_SAVE_PX_PADDING;
    } else if (width !== null && height == null) {
        height =
            (width - 2 * CONF.DEFAULT_SAVE_PX_PADDING) / aspect_ratio +
            2 * CONF.DEFAULT_SAVE_PX_PADDING;
    } else if (width == null && height == null) {
...
function clean_point_attributes(attributes: PointRenderAttributes) {
    ...
    attributes.stroke = interpolate_colors(
        attributes.stroke,
        attributes.stroke
    );
