...
function matrix_from_input_output(
    f_in: [Vector, Vector],
    f_out: [Vector, Vector],
) {
    // Expect Col Vectors
...
function affine_transform_from_input_output(
    f_in: [Vector, Vector],
    f_out: [Vector, Vector],
) {
    // f_in --- f ---> f_out
...
function _convex_hull(points: Vector[]) {
    ...
    for (const p of points) {
        while (
            lower.length >= 2 &&
            lower[lower.length - 2]!.subtract(lower[lower.length - 1]!).cross(
                p.subtract(lower[lower.length - 1]!),
            ) <= 0
        ) {
            lower.pop();
        }
    ...
    for (let i = points.length - 1; i >= 0; i--) {
        ...
        while (
            upper.length >= 2 &&
            upper[upper.length - 2]!.subtract(upper[upper.length - 1]!).cross(
                p.subtract(upper[upper.length - 1]!),
            ) <= 0
        ) {
            upper.pop();
        }
...
function convex_hull(
    points: Vector[],
): Polygon & { contains: (pt: Vector) => boolean } {
...
function polygon_orientation(
    ...
    if (
        new_points[0]!.distance_squared(new_points[new_points.length - 1]!) <
        eps
    ) {
        new_points.pop();
    }
