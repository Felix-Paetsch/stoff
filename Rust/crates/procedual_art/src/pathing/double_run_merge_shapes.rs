use geometry::{
    Polygon, Polyline, Shape, ShapePosition, ShapeT, Vector, closest::LazyClosestShapePositions,
};
use union_find::{QuickFindUf, Union, UnionFind};

pub struct DoubleRunShapeMergingOptions {
...
impl DoubleRunShapeMergingOptions {
    pub fn new(
        max_merge_distance: Option<f64>,
        min_line_amount: Option<usize>,
    ) -> DoubleRunShapeMergingOptions {
        DoubleRunShapeMergingOptions {
...
impl Default for DoubleRunShapeMergingOptions {
    fn default() -> Self {
        DoubleRunShapeMergingOptions {
...
pub fn double_run_merge_shapes(shapes: &[Shape]) -> Shape {
    ...
    let mut res =
        double_run_merge_shapes_with_options(shapes, DoubleRunShapeMergingOptions::default());
...
pub fn double_run_merge_shapes_with_options(
    shapes: &[Shape],
    cfg: DoubleRunShapeMergingOptions,
) -> Vec<Shape> {
    ...
    for _ in 0..shapes.len().saturating_sub(cfg.min_line_amount) {
        if let Some(min_distance_pos) = lazy_min_distances.pop()
            && min_distance_pos.distance() <= cfg.max_merge_distance
        {
            double_run_merge_uf.union(
                min_distance_pos.shape1_index(),
                min_distance_pos.shape2_index(),
            );
            lazy_min_distances
                .retain_lazy(|a, b| double_run_merge_uf.find(a) != double_run_merge_uf.find(b));

            let [pos_a, pos_b] = min_distance_pos.positions();
            merge_positions.push(MergePosition {
                this: min_distance_pos.shape1_index(),
                that: min_distance_pos.shape2_index(),
                own_position: pos_a,
            });
            merge_positions.push(MergePosition {
                this: min_distance_pos.shape2_index(),
                that: min_distance_pos.shape1_index(),
                own_position: pos_b,
            });
        } else {
            break;
