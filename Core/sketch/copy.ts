...
export namespace Copy {
    ...
    export function sketch_element_collection(
        ...
        if (!target_sketch)
            target_sketch = CollectionMethods.get_sketch(source);
        ...
        function get_corresponding_sketch_element(
            el: SketchElement,
        ): SketchElement;
        ...
        CollectionMethods.get_lines(source).forEach((line) => {
            const endpoint_1 = (reference_array.find(
                (v) => v[0] == line.p1,
            )?.[1] ?? line.p1) as Point;
            const endpoint_2 = (reference_array.find(
                (v) => v[0] == line.p2,
            )?.[1] ?? line.p2) as Point;
    ...
    export function line(ln: Line, ...res: any[]): Line {
        ...
        if (res.length == 0 || res[0] == null || res[0] == ln.sketch) {
            ...
            new_ln = new Line(
                [
                    point(ln.p1, res[0] as Sketch),
                    point(ln.p2, res[0] as Sketch),
                ],
                ln.shape,
            );
        }
