...
export function main_pattern_construction(mea, design_data) {
    //Sketch.dev.global_recording().hot_at_url("/hot");
    const shirt = new StageProcess({
        measurements: mea,
        ease: design_data.ease,
    });
    shirt.add_stage(
        new EasyPatternNecklineStage(
            design_data.dartAllocation.one_waistline_dart
        )
    );
    ...
    switch (design_data.neckline["type"]) {
        case "round":
    ...
    switch (design_data.dartAllocation.type) {
        ...
        case "single dart":
            ...
            if (
                design_data.dartAllocation.position == "waistline" ||
                design_data.dartAllocation.position == "outer"
            ) {
                number_of_darts = 0;
        ...
        case "double dart":
            switch (design_data.dartAllocation.position) {
                ...
                case "waistline and french":
                    shirt.multiple_dart_different_location(
                        "waistline",
                        "french"
                    );
                    shirt.remove_waistline_dart("outer");
                ...
                default:
                    ...
                    if (
                        ...
                        design_data.dartAllocation.position == "shoulder"
                    ) {
                        swap_orientation = true;
                    }
                    break;
        ...
        case "multiple darts":
            if (design_data.dartAllocation.position == "waistline") {
                ...
                number_of_darts =
                    design_data.dartAllocation.multiple_darts_number;
                shirt.multiple_dart_one_location(
                    design_data.dartAllocation.position,
                    design_data.dartAllocation.multiple_darts_number
                );
                shirt.remove_waistline_dart("both");
            }
        ...
        case "styleline":
            number_of_darts = 0;
            if (
                design_data.dartAllocation.styleline_type == "classic princess"
            ) {
