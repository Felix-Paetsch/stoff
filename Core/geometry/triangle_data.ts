...
export namespace Triangle {
    ...
    export function from_specification(triangle: Specification): Triangle {
        const { SSA = true, ...input } = triangle as Specification & {
            SSA?: boolean;
        };
        ...
        if (
            [A, B, C, Alpha, Beta, Gamma].filter((v) => v !== undefined)
                .length < 3
        ) {
        ...
        {
            const sides: Record<SideKey, number | undefined> = {
                a: A,
                b: B,
                c: C,
            };
        ...
        do {
            ...
            if (knownAngles.length === 2) {
                ...
                angles[missing] =
                    Math.PI -
                    angles[knownAngles[0]!]! -
                    angles[knownAngles[1]!]!;
                changed = true;
            }
