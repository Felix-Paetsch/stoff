...
use crate::closest::{self, ClosestShapePositionsResult};
use crate::length_map::LengthMap;
use crate::{BoundingBox, Shape, ShapePosition, ShapeT};

#[derive(Debug, Clone)]
pub struct ShapeDistanceDatum(usize, usize, ClosestShapePositionsResult);

impl ShapeDistanceDatum {
    pub fn new(
        shape1_index: usize,
        shape2_index: usize,
        sh_pos_res: ClosestShapePositionsResult,
    ) -> ShapeDistanceDatum {
        ShapeDistanceDatum(shape1_index, shape2_index, sh_pos_res)
    }

    pub fn shape1_index(&self) -> usize {
        self.0
    }

    pub fn shape2_index(&self) -> usize {
        self.1
    }

    pub fn shape1_position(&self) -> ShapePosition {
        self.2.positions[0]
    }

    pub fn shape2_position(&self) -> ShapePosition {
        self.2.positions[1]
    }

    pub fn positions(&self) -> [ShapePosition; 2] {
        self.2.positions
    }
    ...
    pub fn into_closest_shape_positions_result(self) -> ClosestShapePositionsResult {
        self.2
    }
}
...
impl<'a> LazyClosestShapePositions<'a> {
    pub fn new(shapes: &'a [Shape]) -> LazyClosestShapePositions<'a> {
        ...
        let bbs: Vec<_> = shapes
            .iter()
            .map(|s| BoundingBox::from_vectors(s.vertices().iter().copied()).unwrap())
            .collect();
