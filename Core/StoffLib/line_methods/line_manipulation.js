        ...
        while (true){
            ...
            while (to_move_forwards > 0){
                if (latest_point[0] == this.sample_points.length - 1){
                    interp_points.push(this.sample_points[this.sample_points.length - 1]);
                    weights[weights.length - 1] = weights[weights.length-1] + to_move_forwards * weight_per_step;
                    to_move_forwards = 0;
                    break;
                }
                
                let distance_to_next_pt = this.sample_points[latest_point[0]].distance(this.sample_points[latest_point[0] + 1]);
                let rem_distance_to_next_pt = distance_to_next_pt * (1 - latest_point[1]);
                ...
                let split_fraction = latest_point[1] + to_move_forwards / distance_to_next_pt;
                
                const start = this.sample_points[latest_point[0]].scale(1-split_fraction);
                const end = this.sample_points[latest_point[0]+1].scale(split_fraction)
                interp_points.push(
                    start.add(end)
                );
            ...
            while (to_move_backwards > 0){
                ...
                if (interp_points[0] == interp_points[1]){ // We are at the beginning
                    weights[0] = weights[0] - to_move_backwards * weight_per_step;
                    to_move_backwards = 0;
                    break;
                }

                const delta_weight = (weights[0] - to_move_backwards * weight_per_step) / weights[0];
                interp_points[0] = interp_points[0].scale(delta_weight).add(interp_points[1].scale(1 - delta_weight));
