...
export function calculate_cut_groups_no_fixed_point(
    ...
    expect(components.length < 3, "Cant deduce smart cut");
...
export function calculate_cut_groups_with_fixed_point(
    ...
    if (group1 !== null) {
        const other_lines = splitting_points.flatMap((l) =>
            l.get_adjacent_lines(),
        );
        const filtered = other_lines.filter(
            (l) => !lines.includes(l) && !group1.includes(l),
        );
    ...
    expect(relevant_components.length < 3, "Cant deduce smart cut");
