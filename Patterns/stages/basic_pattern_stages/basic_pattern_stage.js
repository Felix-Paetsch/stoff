...
export default class BasicBaseStage extends BaseStage {
    constructor(one_w_dart = true, basic = null, side = "front", needed_base_type = 1) {
        super();
    ...
    #construct_parts(pts, lns, pts2, lns2){
      ...
      if (this.sh.bust > this.sh.waist) {
          ...
          angle = angle + vec_angle_clockwise(
  ...
  //    let ln_h2 = this.sketch.line_between_points(pts.d, pts2.b);
    ...
    //  this.sketch.remove(temp_armpit.point, ln_h2);
      ...
      if(this.needed_base_type == 2){
        ...
        console.log("Das funktioniert noch nicht! Ich kann noch nicht Typ 1 in Typ 3 umwandeln!")
        return;
      }
...
// ToDo: Gefällt mir noch nicht ganz, aber mal schauen
  ...
  //    this.#construct_armpit();
      ...
      if(this.needed_base_type == 1){
        ...
        console.log("Das funktioniert noch nicht! Ich kann noch nicht Typ 2 in Typ 3 umwandeln!")
        return;
      }
    ...
    #change_type_1_into_type_2(){
      ...
      let angle = vec_angle_clockwise(h_to_k.p2.subtract(h), h_to_l.p2.subtract(h));
      ...
      */
      let vec = h_to_i.get_line_vector().normalize().scale(h_to_g.get_length());
    ...
    #change_type_2_into_type_1(){
      ...
      cuted.cut_parts[0].line.p2.other_adjacent_line(cuted.cut_parts[0].line).data.type = "d_to_k";

      cuted.cut_parts[1].line.data.type = "h_to_l";
      cuted.cut_parts[1].line.p2.data.type = "l";
      cuted.cut_parts[1].line.p2.other_adjacent_line(cuted.cut_parts[1].line).data.type = "l_to_c";


      let h_to_g = this.sketch.get_typed_line("h_to_g")
      let h_to_i = this.sketch.get_typed_line("h_to_i")

      let angle = vec_angle_clockwise(h_to_g.p2.subtract(e), h_to_i.p2.subtract(e));
      let fun = rotation_fun(e, angle);
...
// wird extern aufgerufen
    ...
    #add_pattern_type(){
      ...
      f.move_to(f.subtract(new Vector(this.basic_config.width.waistline/4, 0)));

      const o = this.sketch.get_typed_point("o");
      o.move_to(o.subtract(new Vector((this.basic_config.width.waistline + this.basic_config.width.bottom)/8, 0)));

      const n = this.sketch.get_typed_point("n");
      n.move_to(n.subtract(new Vector(this.basic_config.width.bottom/4, 0)));
