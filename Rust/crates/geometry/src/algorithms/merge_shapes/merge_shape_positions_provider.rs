...
use minimum_weight_perfect_matching::min_weight_matching_f64;
use std::{collections::VecDeque, rc::Rc};
use union_find::{QuickFindUf, Union, UnionFind};

use crate::{
    Shape, ShapeT, Vector,
    algorithms::merge_shapes::types::{MergePosition, ShapeEndpoint, ShapeEndpointPairDatum},
    closest::{LazyClosestShapePositions, ShapeDistanceDatum},
        ...
        let merge_into_polyline: VecDeque<Rc<ShapeDistanceDatum>> = {
            ...
            if a.is_empty() {
                ...
                while !a.is_empty() && !b.is_empty() {
                    ...
                    if front_a.distance() == front_b.distance() {
                        ...
                        if Rc::ptr_eq(front_a, front_b) {
                            ...
                            let occurrence_in_b = b
                                .iter()
                                .skip(1)
                                .take_while(|item_b| item_b.distance() == front_a.distance())
                    ...
                    } else if front_a.distance() < front_b.distance() {
...
impl<'a> MergeShapePositionsProvider<'a> {
    ...
    pub fn initialize_with_fixed_endpoints(
        ...
        fixed_endpoints.sort_by(|a, b| b.cmp(a));
        fixed_endpoints.dedup();
    ...
    pub fn pop_if_below_distance(&mut self, mut dist: f64) -> Option<MergePosition> {
        ...
        loop {
            ...
            if self.possible_endpoint_merges.is_empty() {
                if self.polygon_count == 0 || next_suggested_shape_merge.unwrap().distance() > dist
                {
                    return None;
                }
            ...
            let next_suggested_shape_merge_distance =
                next_suggested_shape_merge.unwrap().distance();
    ...
    fn merge_gon_rc(&mut self, pos: Rc<ShapeDistanceDatum>) -> MergePosition {
        debug_assert!(pos.shape1_index() != pos.shape1_index());
        // Exactly one is a polygon and they are disjoint
        debug_assert!(
            self.merged_shapes_uf.find(pos.shape1_index())
                != self.merged_shapes_uf.find(pos.shape2_index())
        );
        debug_assert!(
            self.merged_shapes_uf.get(pos.shape1_index()).is_polygon
                || self.merged_shapes_uf.get(pos.shape2_index()).is_polygon
        );
        debug_assert!(
            !self.merged_shapes_uf.get(pos.shape1_index()).is_polygon
                || !self.merged_shapes_uf.get(pos.shape2_index()).is_polygon
        ...
        self.merged_shapes_uf
            .union(pos.shape1_index(), pos.shape2_index());
        debug_assert!({
            // Check after the merging there are no intershape connections
            let merge_into_polyline: Vec<_> = self
                .merged_shapes_uf
                .get(pos.shape1_index())
                .merge_into_polyline
                .iter()
                .cloned()
                .collect();
            merge_into_polyline.iter().all(|m| {
                self.merged_shapes_uf.find(m.shape1_index())
                    != self.merged_shapes_uf.find(m.shape2_index())
            })
        });
    ...
    fn merge_gon(&mut self, pos: ShapeDistanceDatum) -> Option<MergePosition> {
        debug_assert!(pos.shape1_index() != pos.shape2_index());
        debug_assert!(
            self.merged_shapes_uf.find(pos.shape1_index())
                != self.merged_shapes_uf.find(pos.shape2_index())
        );

        let p0 = pos.shape1_index();
        let p1 = pos.shape2_index();
        ...
        if !close_line_to_polygon {
            ...
            debug_assert!({
                ...
                let merge_into_polylines: Vec<_> = self
                    ...
                    .cloned()
                    .collect();
                merge_into_polylines.iter().all(|m| {
                    self.merged_shapes_uf.find(m.shape1_index())
                        != self.merged_shapes_uf.find(m.shape2_index())
                })
            });
