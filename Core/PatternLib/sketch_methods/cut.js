...
export function cut_with_fixed_point(s, line, fixed_pt, grp1, grp2) {
    assert(
        line.has_endpoint(fixed_pt),
        "Line doesn't have fixed point as endpoint",
    );
    ...
    if (grp1 == "smart") {
        assert(
            adjacent.length == 2,
            `Non-fixed point in a cut has ${adjacent.length} other adjacent lines; expected 2`,
        );
        grp1 = [adjacent[0]];
        grp2 = [adjacent[1]];
    } else if (grp2 == "smart") {
    ...
    */
    ...
    if (grp1 instanceof Array && !(grp2 instanceof Array)) {
        ...
        grp2 = [
            ...line.p1
                .get_adjacent_lines()
                .filter((l) => !grp1.includes(l) && l !== line),
            ...line.p2
                .get_adjacent_lines()
                .filter((l) => !grp1.includes(l) && l !== line),
        ];
        ...
        assert(
            line.p1.get_adjacent_lines().length == 3 &&
                line.p2.get_adjacent_lines().length,
            "Can't automatically detect which lines group together",
        );
        ...
        grp1 = [line.p1.other_adjacent_lines(line)[0]];
        {
            ...
            while (visited_points.indexOf(current_line_endpoint) < 0) {
                ...
                current_line =
                    current_line_endpoint.other_adjacent_line(current_line);
                current_line_endpoint = current_line.other_endpoint(
                    current_line_endpoint,
                );
            }
            assert(
                current_line_endpoint === line.p2,
                "Can't automatically detect which lines group together",
            );
    ...
    const { cut_parts: cut_parts_1 } = cut_with_fixed_point(
        s,
        line,
        fixed_1,
    ...
    const { cut_parts: cut_parts_2 } = cut_with_fixed_point(
        s,
        pre_connecting_line,
        fixed_2,
    ...
    return s.object_to_sketch_element_collection({
        points: s.make_sketch_element_collection(
            line1.get_endpoints().concat(line2.get_endpoints()),
        ),
    ...
    if (typeof grp1 !== "array") {
        ...
        for (let i = 0; i < endpoints.length; i++) {
            grp2.concat(
                ...endpoints[i]
                    .get_adjacent_lines()
                    .filter((l) => grp1.indexOf(l) < 0 && path.indexOf(l) < 0),
            );
        }
    }
...
function _component_cut_along_line_path(s, component, path, grp) {
    for (let i = 0; i < path.length; i++) {
        if (path[i].p1 == component.points[i][1]) {
            component.lines.push(
                s.copy_line(
                    path[i],
                    component.points[i][0],
                    component.points[i + 1][0],
                ),
            );
        } else {
            component.lines.push(
    ...
    for (let i = 0; i < grp.length; i++) {
        ...
        throw new Error(
            "Group line doesn't have an endpoint among the line_path.",
        );
    }
}
