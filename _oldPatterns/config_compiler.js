export default function config_compiler(config) {
  ...
  if (
    conf.dartAllocation.type == "single dart" ||
    conf.dartAllocation.type == "multiple darts"
  ) {
    if (
      ![
        "waistline",
      ...
      throw new Error(
        "Selected type doesn't match 'single dart' or 'multiple darts'",
      );
    }
  ...
  if (conf.dartAllocation.type == "multiple darts") {
    conf.dartAllocation.multiple_darts_number =
      config["top designs"]["number of multiple darts"];
  }
