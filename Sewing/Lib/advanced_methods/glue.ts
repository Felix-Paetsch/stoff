...
export function glue(
    ...
    ident2: GlueIdent,
    data: {
        points: "delete" | Copy.CopySketchObjectDataCallback;
        lines: "delete" | "keep" | Copy.CopySketchObjectDataCallback;
    },
): GlueResult {
...
function glue_with_fixed_point(
    ...
    Expect.that(ident1[1] !== ident2[1], "Already glued together");

    const fixed = ident1[0];
    const p1 = ident1[1];
    const p2 = ident2[1];

    const sketch_as_collection =
        CollectionMethods.sketch_element_collection_as_array(fixed.sketch);
    const total_connected_component_points = CollectionMethods.get_points(
        CollectionMethods.connected_component(sketch_as_collection, fixed),
    );
    Expect.that(
    ...
    const avoidant_cc2_points = CollectionMethods.get_points(
        CollectionMethods.connected_component(
            sketch_as_collection.filter((e) => e !== fixed),
            p2,
        ),
    );
    ...
    const transform_fun = LinearTransform.affine_orthogonal(
        [fixed.vec, p2.vec],
        [fixed.vec, p1.vec],
    );
    avoidant_cc2_points.forEach((pt) => pt.move_to(transform_fun(pt.vec)));
    ...
    let glue_lines =
        CollectionMethods.lines_by_key(s, "__glue_line")["true"] ||
        merged_pt.common_lines(fixed);
    ...
    if (data.points == "delete") {
        delete_glue_point(
            s,
            merged_pt,
            data.lines as Copy.CopySketchObjectDataCallback,
        );
    }
...
function glue_without_fixed_point(
    ...
    const cc2 = CollectionMethods.connected_component(
        ident2[0].sketch,
        ident2[0],
    ...
    Expect.that(cc2.includes(ident2[1]), "Glue ident isn't connected");

    const transform_fun = LinearTransform.affine_orthogonal(
        [ident2[0].vec, ident2[1].vec],
        [ident1[1].vec, ident1[1].vec],
    );
    cc2.forEach((pt) => pt.move_to(transform_fun(pt.vec)));
...
function delete_glue_point(
    s: Sketch,
    pt: Point,
    line_callback: Copy.CopySketchObjectDataCallback,
) {
    const adjacent = pt.adjacent_lines();
