...
import { Vector, triangle_data, rotation_fun, vec_angle_clockwise , vec_angle, deg_to_rad, rad_to_deg, DOWN, LEFT } from "../../../Core/StoffLib/geometry.js";
...
export default class BasicSleeveBaseStage extends BaseStage{
    ...
    #basic_straight_construction(){
        ...
        pts.b = new Vector(0, this.wd.measurements.distance_armpit * this.wd.mult_vector);
        pts.c = pts.b.add(new Vector(0, this.wd.measurements["arm length"]));


        pts.d = this.sketch.point(pts.b.add(new Vector(this.wd.measurements["arm"] / 2, 0)));
        pts.e = this.sketch.point(pts.b.add(new Vector(-this.wd.measurements["arm"] / 2, 0)));

        pts.f = this.sketch.point(pts.c.add(new Vector(this.wd.measurements["arm"] / 2, 0)));
        pts.g = this.sketch.point(pts.c.add(new Vector(-this.wd.measurements["arm"] / 2, 0)));
        ...
        lns.a_to_d = this.sketch.line_with_length(pts.a, pts.d, pts.a.subtract(pts.d).length() + 1);
        lns.a_to_e = this.sketch.line_with_length(pts.e, pts.a, pts.a.subtract(pts.e).length() + 2).mirror();
...
//
    cut_sleeve_stripes(number_of_stripes = 3){
        ...
        const vec = this.sketch.get_typed_line("side").get_line_vector().scale(-2).add(new Vector(0, -this.wd.measurements.distance_armpit * 2));

        let temp = this.sketch.split_line_at_length(bottom, len);
        for (let i = 0; i < number_of_stripes - 1; i++) {

            lns.push(this.sketch.line_at_angle(temp.point, deg_to_rad(0), vec.length()).line);
            ...
            if(i < number_of_stripes - 2){
                temp = this.sketch.split_line_at_length(temp.line_segments[1], len);
            }
    ...
    flare_bottom_sleeve(distance_armpit = 5, flare_by_angle = false){
        ...
        dart_lines.sort(function(a,b){return a.data.dart_number - b.data.dart_number});
        ...
        dart_lines.forEach((l, index) => {
            ...
            const bottom_right_corner = top_right_corner.other_adjacent_point(top_left_corner, prev_top_right_corner, prev_bottom_left_corner);
            ...
            dart_lines[index] = top_left_corner.common_line(bottom_left_corner).set_orientation(
            ...
            for (let i = index; i < dart_lines.length; i++){
                ...
                [bottom_left_corner, bottom_right_corner, top_right_corner].forEach(
                    p => p.move_to(rot_fun(p))
                );
            }
        ...
        const rot_correction_fun = rotation_fun(correction_pivot, correction_angle);
    ...
    flare_top_sleeve(distance_armpit = 5, flare_by_angle = false) {
        ...
        */
        ...
        let bottom_right_corner = this.sketch.get_point_between_lines(
            (l) => { return l.data.type == "side" && l.data.side == "front" },
            (l) => { return l.data.type == "bottom" && l.p1.data.type == "f" }
        );

        dart_lines.forEach((l, index) => {
            //    let top_left_corner = l.p1;
            const bottom_left_corner = l.p2;
            let top_right_corner = bottom_right_corner.other_adjacent_point(bottom_left_corner, prev_bottom_right_corner, prev_top_left_corner);
            ...
            for (let i = index; i < dart_lines.length; i++) {
                ...
                [top_left_corner, bottom_right_corner, top_right_corner].forEach(
                    p => p.move_to(rot_fun(p))
                );
            }
    ...
    connect_sleeve_bottom(middle_addition = 3){
      ...
      //  let lns = this.sketch.get_typed_lines("bottom");
        ...
        const vec = sides[0].p1.subtract(sides[1].p1).scale(0.5).add(sides[1].p1);
        let pts = [this.sketch.get_typed_point("g")];
        const step_size = middle_addition / Math.log(Math.round((lns.length - 1) / 2));
...
*/
        ...
        const curve = this.sketch.line_from_function_graph(pts[0], pts[pts.length - 1], spline.catmull_rom_spline(pts));
        /*
        */
        delete pts[0];
    ...
    connect_sleeve_top(middle_addition = 3) {
        ...
        lns.sort(function(a, b) {
            return a.p2.other_adjacent_line(a).data.dart_number - b.p2.other_adjacent_line(b).data.dart_number;
        });
        let sides = this.sketch.get_typed_lines("side");
        const vec = sides[0].p1.subtract(sides[1].p1).scale(0.5).add(sides[1].p1);
...
// Wird direkt nach der konstruktion des Ärmels aufgerufen, bevor irgendwas anderes gemacht wird
    ...
    cut_to_other_sketch(length){
        ...
        const p1 = this.sketch.split_line_at_length(bottom.p1.other_adjacent_line(bottom), len);
        const p2 = this.sketch.split_line_at_length(bottom.p2.other_adjacent_line(bottom), len);
...
*/
    ...
    slim_sleeve(sum_reduction = 2){
        ...
        dart_lines.forEach(line => {
            ...
            this.sketch.glue(cut.cut_parts[0].line, cut.cut_parts[1].line, {points: "delete_both", lines: "delete", anchors: "keep"})
        });
    ...
    flare_straight(sum_flare_distance = 2){
        ...
        for (let i = 0; i < number_of_stripes - 1; i++) {
            ...
            new ConnectedComponent(cuts[i].cut_parts[0].line).transform(p => {p.move_to(p.add(new Vector(-flare_distance,0)))});
        ...
        armpit.sort((a, b) => { return a.p1.x - b.p1.x; });
        ...
        const curve = this.sketch.line_from_function_graph(pts[0], pts[pts.length - 1], spline.catmull_rom_spline(
            pts
        ));
