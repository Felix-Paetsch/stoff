...
export type InternalGraph = LengthGraph | VertexGraph | ShapeGraph;

export function internal_is_length_graph(g: InternalGraph): g is LengthGraph {
    return g.edges.length == 0 || typeof g.edges[0]!.data == "number";
}

export function internal_is_vertex_graph(g: InternalGraph): g is VertexGraph {
    if (g.nodes.length == 0) return true;
    return g.nodes[0]!.data instanceof Vector;
}

export function internal_is_shape_graph(g: InternalGraph): g is ShapeGraph {
    if (g.nodes.length == 0) return true;
