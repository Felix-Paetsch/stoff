...
#[wasm_bindgen]
pub fn wasm_pathing_double_run_vertex_graph(graph: &WASMVectorUnitGraph) -> WASMShapeCollection {
    let gons = double_run_vertex_graph(graph.inner());
    WASMShapeCollection::promote(gons.into_iter().map(Shape::Polygon).collect())
}

#[wasm_bindgen]
pub fn wasm_pathing_double_run_shape_graph(graph: &WASMVectorShapeGraph) -> WASMShapeCollection {
